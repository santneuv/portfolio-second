...
float displacement(vec3 p) {
  float n = snoise(p * uFrequency + uTime * 0.3);
  float detail = snoise(p * uFrequency * 2.5 - uTime * 0.2) * 0.35;
  return (n + detail) * uAmplitude;
}
...
void main() {
  ...
  // Recompute the normal from two displaced neighbours along the surface.
  ...
  vec3 da = pa + n * displacement(pa);
  vec3 db = pb + n * displacement(pb);
